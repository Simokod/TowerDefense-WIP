...
func setup(seleced_hero: BaseHero, _heroes_selection_ui: HeroesSelectionUI) -> void:
	...
	texture_rect.position = (position - Vector2(tilemap.tile_set.tile_size) / 2)
	texture_rect.mouse_filter = Control.MOUSE_FILTER_IGNORE
	hero_sprite = texture_rect
	add_child(texture_rect)
	
	hero_sprite.texture = hero.unit_sprite
...
func _process(_delta: float) -> void:
	...
	var tile_center_position = tilemap.map_to_local(tile_position)
